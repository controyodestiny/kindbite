...
# Try multiple locations for .env file (useful for different deployment scenarios)
env_paths = [
    BASE_DIR / '.env',  # In backend directory
    BASE_DIR.parent / '.env',  # In project root
    Path.home() / '.env',  # In user home (PythonAnywhere)
]
for env_path in env_paths:
    ...
    # If no .env file found, still try to load (might use system env vars)
    load_dotenv()
...
# Load from environment; fall back only for local dev
SECRET_KEY = os.environ.get('SECRET_KEY', 'dev-only-insecure-secret')

# SECURITY WARNING: don't run with debug turned on in production!
DEBUG = os.environ.get('DEBUG', 'True').lower() == 'true'

ALLOWED_HOSTS = os.environ.get('ALLOWED_HOSTS', 'localhost,127.0.0.1,kindbite.pythonanywhere.com').split(',')
...
TEMPLATES = [
    {
        ...
        'DIRS': [
            # Serve React build index.html as a template
            (Path(__file__).resolve().parent.parent.parent / 'build'),
            # Also allow serving from collected staticfiles path
...
STATICFILES_DIRS = [
    # Include React build static assets
...
# CSRF trusted origins for hosted domain
